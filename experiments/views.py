...
from .experiments import (create_new_experiment, delete_existing_experiment,
                          experiment_state_change,
                          generate_experiment_session_request,
                          get_emulab_manifest, get_experiment_list,
                          initiate_emulab_instance, is_emulab_stage,
                          query_emulab_instance_status,
...
def experiment_update_experimenters(request, experiment_uuid):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            experiment.experimenter.through.objects.filter(
    ...
    return render(
        request,
        "experiment_update_experimenters.html",
        {
            "form": form,
...
def experiment_update_by_ops(request, experiment_uuid):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            )  # save first, otherwise soon overwritten
            experiment = form.save(commit=False)
            experiment_uuid = update_existing_experiment(
                request, experiment, form, prev_stage, prev_state
            )
...
def experiment_manifest(request, experiment_uuid):
    ...
    if (
        experiment.stage == "Development"
        and experiment.state == Experiment.STATE_DEPLOYED
