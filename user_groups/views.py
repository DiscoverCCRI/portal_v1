...
def user_groups(request):
    ...
    user = request.user
    all_users = (
        AerpawUser.objects.all().exclude(username="admin").order_by("display_name")
    )
...
def user_requests(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if is_approved:
                messages.info(
                    request,
                    "Success! APPROVED: Request to add role: "
                    + r_name
                    + " has been sent",
                )
            else:
                messages.info(
                    request,
                    "Success! DENIED: Request to add role: "
                    + r_name
                    + " has been sent",
                )
    ...
    if user_manager.is_superuser:
        print("IS SUPERUSER")
        open_u_reqs = AerpawRoleRequest.objects.filter(is_completed=False).order_by(
            "-created_date"
        )
        closed_u_reqs = AerpawRoleRequest.objects.filter(is_completed=True).order_by(
            "-created_date"
        )
    elif user_manager.is_site_admin():
        print("IS SITE ADMIN")
        open_u_reqs = (
            AerpawRoleRequest.objects.filter(is_completed=False)
            .exclude(requested_role__in=sa_set)
            .order_by("-created_date")
        )
        closed_u_reqs = (
            AerpawRoleRequest.objects.filter(is_completed=True)
            .exclude(requested_role__in=sa_set)
            .order_by("-created_date")
        )
    else:
        ...
        open_u_reqs = (
            AerpawRoleRequest.objects.filter(is_completed=False)
            .exclude(requested_role__in=um_set)
            .order_by("-created_date")
        )
        closed_u_reqs = (
            AerpawRoleRequest.objects.filter(is_completed=True)
