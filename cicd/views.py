...
def cicd_create(request):
    ...
    return render(
        request,
        "cicd_create.html",
        {"form": form, "is_pc": is_pc, "is_po": is_po, "is_pm": is_pm},
    )
...
def cicd_detail(request, cicd_uuid):
    ...
    try:
        ...
        project = {"name": "UNDEFINED", "uuid": cicd.aerpaw_uuid}
    if request.GET.get("deploy_cicd"):
        ...
        status = {"message": info, "timestamp": timezone.now()}
...
def cicd_host_info_detail(request, cicd_host_info_uuid):
    ...
    if request.GET.get("purge_cicd_host_info"):
        if not cicd_hi.is_allocated:
            ...
            messages.error(
                request,
                "ERROR: Unable to delete an allocated CI/CD resource connection",
            )
            return redirect(
                "cicd_host_info_detail", cicd_host_info_uuid=str(cicd_hi.uuid)
            )
...
def cicd_host_info_update(request, cicd_host_info_uuid):
    ...
    if cicd_hi.is_allocated:
        messages.info(
            request, "INFO: Unable to edit because CI/CD resource is already allocated"
        )
        return render(request, "cicd_host_info_detail.html", {"cicd_hi": cicd_hi})
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                "cicd_host_info_detail", cicd_host_info_uuid=str(cicd_host_info_uuid)
            )
        else:
            messages.error(request, "Error")
    else:
    ...
    return render(
        request,
        "cicd_host_info_update.html",
        {"form": form, "cicd_hi_uuid": str(cicd_hi.uuid)},
    )
