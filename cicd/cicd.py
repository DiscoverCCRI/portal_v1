...
def create_new_cicd(request, form, project_uuid):
    ...
    if not cicd.cicd_host_info:
        messages.error(request, 'ERROR: All available CI/CD resource connections have already been allocated, \
...
def create_new_cicd_host_info(request, form):
    ...
    cicd_hi.name = str(cicd_hi.fqdn_or_ip) + ' (' + str(cicd_hi.nginx_http_port) + ' / ' + \
    ...
    """.format(cicd_hi.fqdn_or_ip, cicd_hi.nginx_http_port, cicd_hi.nginx_https_port, cicd_hi.name,
               cicd_hi.docker_subnet, cicd_hi.jenkins_service_agent_port, cicd_hi.jenkins_ssh_agent_port,
